...
export const JobCardDetailsScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  // Fetch custom parts when picker opens
  useEffect(() => {
  ...
  const addPart = (name: string, isCustom?: boolean) => {
    setEditPartLines(prev => [
      ...prev,
      { id: `${Date.now()}-${Math.random()}`, name: name, isCustom: isCustom || false, cost: '' },
    ]);
            ...
            return (
              <>
                {partSearch.trim().length > 0 && !hasAnyResults && (
                  <TouchableOpacity
                    onPress={() => addPart(partSearch.trim(), true)}
