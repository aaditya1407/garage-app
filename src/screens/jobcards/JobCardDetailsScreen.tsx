...
export const JobCardDetailsScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  const executeStatusUpdate = async (newStatus: string) => {
    ...
    try {
      ...
      const { error } = await supabase.from('job_cards').update(payload).eq('id', jobId);
      if (error) throw error;
      fetchJobDetails();
    } catch (err) {
          ...
          {editFinalNote.trim().length > 0 && (
            <Button
              ...
              onPress={async () => {
                setSaving(true);
                try {
                  const { error } = await supabase.from('job_cards').update({ final_note: editFinalNote }).eq('id', jobId);
                  if (error) throw error;
                  fetchJobDetails();
                } catch (err: any) {
